...
const TONE: Record<string, string> = {
  ...
  수용: "border-emerald-600/30 bg-emerald-600/5 text-emerald-700",
};

function toneOf(decision: string): string {
  const key = Object.keys(TONE).find((k) => decision.includes(k));
  return key ? TONE[key] : "border-line bg-white text-ink";
}

export function VerdictBanner({ verdict }: { verdict: Stage2Result["verdict"] }) {
  return (
    <div className={`rounded-xl border p-5 ${toneOf(verdict.decision)}`}>
